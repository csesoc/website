...
func TestEffectIndependence(t *testing.T) {
	...
	pos3 := []int{1, 2}
	pos4 := []int{1, 0, 3, 2}
	assert.Equal(t, editor.TransformPoint(pos3, pos4), true)

	// Effect independence condition 3
	pos5 := []int{1, 0, 3, 2}
	pos6 := []int{1, 2}
	assert.Equal(t, editor.TransformPoint(pos5, pos6), true)

	// Not effect indepenent
	pos7 := []int{1, 2, 3, 4}
	pos8 := []int{1, 2, 3, 4}
	assert.Equal(t, editor.TransformPoint(pos7, pos8), false)
}
...
func TestTransformDeletes(t *testing.T) {
	...
	pos9, pos10 = editor.TransformInserts(pos9, pos10, TP5)
