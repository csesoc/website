...
const constructors: Record<string, (props: CMSBlockProps) => JSX.Element> = {
  ...
  'list-item': (props) => <EditorBlock {...props} />,
  heading: (props) => <HeadingBlock {...props} />,
  'unordered-list': (props) => <EditorBlock {...props} />,
  'ordered-list': (props) => <EditorBlock {...props} />,
  quote: (props) => <EditorBlock {...props} />,
};
 ...
 */
export const buildComponentFactory =
  (
    opManager: OperationManager,
    onClick: (id: number) => void,
    onUpdate: UpdateCallback
  ) =>
  (block: BlockData, blockId: number, isFocused: boolean): JSX.Element => {
...
const buildUpdateHandler =
  (
    blockId: number,
    opManager: OperationManager,
    updateCallback: callbackHandler
  ) =>
  (id: number, editorContent: BlockData, operations: BaseOperation[]) => {
    ...
    opManager.pushToServer(
      slateToCmsOperation(editorContent, modifiedOperations)
    );
  };
